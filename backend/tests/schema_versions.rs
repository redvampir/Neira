...
fn load_multiple_schema_versions() {
    ...
    let original = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../schemas/v1/node-template.schema.json");
    for ver in &["v1", "v2"] {
        ...
        fs::write(dir.join("node-template.schema.json"), &content).expect("write schema");
    }
    std::env::set_var("NODE_TEMPLATE_SCHEMAS_DIR", base);
