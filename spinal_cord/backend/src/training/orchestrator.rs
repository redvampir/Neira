    ...
    async fn run_cycle(self: Arc<Self>) -> MicrotaskResult {
        ...
        match result {
            ...
            Err(err) => {
                ...
                error!("?????? ?????????? ?????: {}", err);
                let severity = if current >= self.max_failures {
                    IncidentSeverity::Critical
                } else {
                    IncidentSeverity::High
                };
                let incident = HealingIncident::new(
                    format!("training.orchestrator.{}", self.id),
                    "?????????? ???????? ??????????? ???????",
                )
                .with_scope(IncidentScope::Runtime)
                .with_severity(severity)
                .add_signal(format!("failures={current}"))
                .add_signal(format!("max_failures={}", self.max_failures))
                .with_details(err.clone());
                if let Some(outcome) = healing_sleep::record_global_incident(incident).await {
                    metrics::counter!(
                        "training_orchestrator_healing_events_total",
                        "reason" => "cycle_failure"
                    )
                    .increment(1);
                    debug!(?outcome, "healing sleep handled orchestrator failure");
                }
