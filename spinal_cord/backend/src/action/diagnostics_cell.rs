...
use crate::healing_sleep::{self, HealingIncident, IncidentScope, IncidentSeverity};
            ...
            while let Some(record) = rx.recv().await {
                ...
                if let Some(cred) = record.metrics.credibility {
                    ...
                    if let Some(alert) = detect_anomaly(&slice[..]) {
                        ...
                        if let Some(outcome) = healing_sleep::record_global_incident(
                            HealingIncident::new(
                                format!("diagnostics.anomaly.{}", record.id),
                                message,
                            )
                            .with_scope(IncidentScope::Runtime)
                            .with_severity(IncidentSeverity::Medium)
                            .add_signal(format!("credibility={cred:.3}")),
                        )
                        .await
                        {
                            metrics::counter!(
                                "diagnostics_cell_healing_events_total",
                                "reason" => "anomaly"
                            )
                            .increment(1);
                            debug!(?outcome, id=%record.id, "healing sleep handled diagnostics anomaly");
                        }
                    }
                    if cred < 0.5 {
                        ...
                        if count >= cell_clone.error_threshold {
                            ...
                            if let Some(outcome) = healing_sleep::record_global_incident(
                                HealingIncident::new(
                                    format!("diagnostics.threshold.{}", record.id),
                                    format!(
                                        "credibility {} below threshold {}",
                                        cred, cell_clone.error_threshold
                                    ),
                                )
                                .with_scope(IncidentScope::Runtime)
                                .with_severity(IncidentSeverity::High)
                                .add_signal(format!("credibility={cred:.3}"))
                                .add_signal(format!("threshold={}", cell_clone.error_threshold)),
                            )
                            .await
                            {
                                metrics::counter!(
                                ...
                                .increment(1);
                                debug!(?outcome, id=%record.id, "healing sleep handled threshold breach");
                            }
