...
impl ToneStateController {
    ...
    pub fn apply_feedback(&self, feedback: ToneFeedback) -> ToneSnapshot {
        match feedback {
            ToneFeedback::Observation { score } => {
                ...
                self.mutate_state(
                    ...
                    |state, now, now_ms| {
                        ...
                        let new_intensity =
                            ((state.intensity * 0.6) + (magnitude * 0.5)).clamp(0.0, 1.0);
                        let new_confidence =
            ...
            ToneFeedback::Decay => {
                self.mutate_state(ToneEventReason::Decay, None, None, true, |_, _, _| false)
            }
        }
    }
    ...
    fn apply_decay_locked(&self, state: &mut ToneInner, now: Instant, now_ms: i64) -> bool {
