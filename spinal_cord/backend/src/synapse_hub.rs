...
use crate::event_bus::{CellCreated, CurriculumLoaded, EventBus, OrganBuilt, Subscriber};
...
impl SynapseHub {
    ...
    */
    ...
    pub fn train_russian_literacy(
        ...
        self.memory.store_parsed_input(ParsedInput::Json(payload));
        hearing::info(&format!(
        ...
        if !interaction_verbs.is_empty() {
            ...
            for verb in &interaction_verbs {
                ...
                hearing::info(&format!(
                    ...
                    chat_id,
                    sid_for_events.as_deref().unwrap_or("<none>"),
                    verb.as_str()
                ));
