...
use backend::healing_sleep::{HealingSleep, HealingSleepConfig};
use backend::voice::{
...
async fn main() {
    ...
    */
    ...
    let healing_config_path = std::env::var("HEALING_SLEEP_CONFIG")
        .unwrap_or_else(|_| "spinal_cord/config/healing_sleep.toml".into());
    let healing_sleep = Arc::new(
        HealingSleep::from_config_file(&healing_config_path, None).unwrap_or_else(|err| {
            warn!(
                %err,
                "не удалось загрузить конфигурацию исцеляющего сна, используем настройки по умолчанию"
            );
            HealingSleep::new(HealingSleepConfig::default(), None)
        }),
    );
    HealingSleep::register_global(healing_sleep.clone());
    ...
    let mut app = Router::new()
        .route(
            "/",
            get(|| async {
                match std::fs::read_to_string("spinal_cord/static/index.html") {
                    Ok(s) => (
                    ...
                    Err(_) => (
                        ...
                        String::from("index.html not found"),
                    ),
                }
            }),
        )
