...
impl LearningMetrics {
    pub fn new() -> Self {
        ...
        let difficulty_level = Gauge::new("current_difficulty", "Текущий уровень сложности").unwrap();

        registry.register(Box::new(success_rate.clone())).unwrap();
        registry.register(Box::new(attempts.clone())).unwrap();
        registry.register(Box::new(difficulty_level.clone())).unwrap();
    ...
    fn get_storage_path(&self) -> Result<PathBuf, Box<dyn StdError>> {
        let base = env::var("NEIRA_DATA_DIR")
