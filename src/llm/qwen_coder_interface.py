...
class QwenCoderLLM(BaseLLM):
    """Wrapper around a local Qwen Coder GGUF model."""
    ...
    def _load_model(self) -> None:
        ...
        if self.model is not None:
            return
        if Llama is None:
            self._load_error = "llama_cpp is required to use QwenCoderLLM"
        ...
        try:
            ...
            logger.info("Qwen Coder model loaded successfully")
            self._load_error = None
        except Exception as exc:  # pragma: no cover - error handling
            self._load_error = str(exc)
            logger.exception("Failed to load Qwen Coder model: %s", exc)
            self.model = None
...
LLMFactory.register("qwen_coder", QwenCoderLLM)
