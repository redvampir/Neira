# src/core/neyra_brain.py - Улучшенная версия
"""
    ...
    def process_command(self, text: str) -> str:
        ...
        for tag in tags:
            if tag.type == 'neyra_command':
                response_parts.append(self._execute_neyra_command(tag.content))
            elif tag.type == 'character_work':
                response_parts.append(self._work_with_character(tag.content))
            elif tag.type == 'emotion_paint':
                response_parts.append(self._add_emotion(tag.content))
            elif tag.type == 'style_guide':
                response_parts.append(self._apply_style(tag.content))
            else:
                response_parts.append(f"🤔 Понимаю тег '{tag.type}', но пока учусь его выполнять...")
