...
`AnalysisCell` (или общий оркестратор) анализирует ввод пользователя в реальном времени и при обнаружении слов‑триггеров передаёт команду `ActionCell`. `ActionCell` инициирует предварительную загрузку связанных узлов памяти, экономя время на последующих этапах. Подробнее см. раздел [«Детекция триггеров»](analysis-nodes.md#детекция-триггеров) файла `analysis-nodes.md`.
