...
## SynapseHub
Единый оркестратор, через который проходят все запросы пользователя. Он регистрирует события, проверяет права доступа и направляет обращения к нужным клеткам. SynapseHub агрегирует ответы ActionCell и MemoryCell, связывая их с исходным контекстом и формируя итоговый результат.
...
### Формат ответа
SynapseHub возвращает объект с итоговым результатом и идентификатором запроса:
...
Клетки, которым необходим конкретный ускоритель (GPU, TPU, FPGA), добавляют в метаданные поле `required_accelerator`. Это позволяет `TaskScheduler` направлять задачи на подходящие устройства:
...
Для задач класса `standard` и `long` SynapseHub отправляет промежуточные сообщения о ходе выполнения. Scheduler выдаёт процент готовности, который отображается пользователю. После завершения задача помечается в журнале и из рабочей памяти удаляются временные записи.
