...
Для проверки целостности файлов можно задать переменную окружения `INTEGRITY_ROOT`,
указывающую базовый каталог для конфигурации и проверяемых путей. По умолчанию
используется текущий рабочий каталог, поэтому при запуске сервиса вне `backend/`
необходимо явно указать `INTEGRITY_ROOT`, иначе проверка не найдёт конфигурацию
и файлы.
...
- **Сбор метрик и диагностика** — фиксирует показатели качества,
  ...
  превышении порога. Подробнее см. [документ](docs/metrics_nodes.md).
