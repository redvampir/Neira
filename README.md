...
но в большинстве случаев её не нужно задавать вручную. При старте узел
[BasePathResolverNode](docs/nodes/action-nodes.md#basepathresolvernode) ищет
`config/integrity.json` вверх по иерархии каталогов и сохраняет найденный путь,
а [InitConfigNode](docs/nodes/action-nodes.md#initconfignode) устанавливает переменную
