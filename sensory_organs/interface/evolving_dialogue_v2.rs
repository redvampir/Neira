/* neira:meta
id: NEI-20251103-evolving-dialogue-v2
intent: feature
summary: |
  Эволюционирующая система диалога — интеграция реальной SemanticMemory из backend.
  Полный цикл: recall → learn → understand → generate → assess → remember → reflect → grow.
*/
...
use backend::memory::{SemanticMemory, SearchResult};
...
impl EvolvingDialogue {
    ...
    /// 8 шагов: recall → learn → understand → generate → assess → remember → reflect → grow
    ...
    async fn track_growth(
        ...
        if tracker.total_conversations == 1 {
            tracker.milestones.push(GrowthMilestone {
                id: format!("milestone_{}", tracker.milestones.len()),
                timestamp: Utc::now(),
            ...
            tracker.milestones.push(GrowthMilestone {
                ...
                milestone_type: MilestoneType::ReachedConversationCount {
                    count: tracker.total_conversations,
                },
                description: format!("Провела {} диалогов!", tracker.total_conversations),
            });
        }
